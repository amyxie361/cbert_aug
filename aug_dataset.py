...
def run_aug(args, save_every_epoch=False):
    ...
    for e in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            init_ids, _, input_mask, segment_ids, _, qa_ids = batch
            ...
            for ids, idx, preds, seg, qa_id in zip(init_ids, masked_idx, predictions, segment_ids, qa_ids):
                ...
                new_str = tokenizer.convert_ids_to_tokens(ids.cpu().numpy())
                new_str = rev_wordpiece(new_str)
                tsv_writer.writerow([new_str, seg[0].item(), qa_id])
            torch.cuda.empty_cache()
